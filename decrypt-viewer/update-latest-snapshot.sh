#!/bin/bash -eu
# Keeps /run/teslacam-latest/mnt pointed at the newest teslausb snapshot,
# so the decryptor/viewer always reads current data without needing to
# know snapshot numbers. teslausb rotates snap-000000, snap-000001, ...
# under /backingfiles/snapshots/ (see run/make_snapshot.sh upstream).
