class FileBrowser {
  ...
  fileClicked(event, path) {
    ...
    if (this.isPlayable(path)) {
      ...
      // Image preview (e.g. LightShow/Wraps PNGs). Served via download.sh,
      // which returns the correct MIME type. Esc or click-outside closes.
      const slash = path.lastIndexOf("/");
      const dir = this.root_path + (slash >= 0 ? "/" + path.substring(0, slash) : "");
      const file = slash >= 0 ? path.substring(slash + 1) : path;
      const src = "cgi-bin/download.sh?" + encodeURIComponent(dir) + "&" + encodeURIComponent(file);
      const div = document.createElement("div");
      div.style.cssText = "position:absolute;width:100%;height:100%;left:0;top:0;background:#000c;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:1000;";
      const close = () => { div.remove(); document.removeEventListener("keydown", onKey); };
      const onKey = (e) => { if (e.key === "Escape") close(); };
      div.onclick = (e) => { if (e.target === div) close(); };
      document.addEventListener("keydown", onKey);
      div.innerHTML =
        '<div style="color:#fff;margin-bottom:8px;max-width:90%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
        displaypath + '</div>' +
        '<img src="' + src + '" style="max-width:92%;max-height:82%;object-fit:contain;background:#222;box-shadow:0 0 20px #000;">';
      document.firstElementChild.append(div);
    }
  }
