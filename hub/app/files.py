...
move / delete under the mounted Media partition (Music/LightShow/Boombox as
subfolders of it -- see run/auto.www). Every path is confined under FS_BASE
(no traversal). The car-written partition is mounted rw by teslausb's
autofs, so no root remount is needed here.
"""
import os, shutil, subprocess

FS_BASE = "/var/www/html/fs"   # teslausb mounts Media (Music/LightShow/Boombox inside it) here
IMAGE_EXT = (".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp")
AUDIO_EXT = (".mp3", ".wav", ".m4a", ".ogg", ".flac", ".aac")
# The partition is an indirect autofs mount: it only appears once accessed, so
# a listdir of FS_BASE shows nothing. Probe the known name instead.
KNOWN_ROOTS = ("Media",)
...
def set_lockchime(rel):
    """Copy a chime file onto Media/Boombox/LockChime.wav -- the exact file
    the car plays on lock/unlock -- overwriting it. Source must live under
    Media/Boombox/ and already meet Tesla's own requirements for that file
    (.wav, <=1MB), since the copy just becomes the new LockChime.wav
    verbatim.

    media_disk.bin (the image backing this whole mount, Music/LightShow/
    Boombox consolidated onto one partition) is also exported live as a raw
    USB mass-storage LUN to the car whenever the gadget is connected (see
    /sys/kernel/config/usb_gadget/teslausb/functions/mass_storage.0/lun.1).
    ...
    if not rel_norm.startswith("Media/Boombox/"):
        raise ValueError("Quelle muss im Media/Boombox-Ordner liegen")
    ...
    dest = _safe("Media/Boombox/LockChime.wav")
    tmp = dest + ".tmp"
